...
int main(int argc, char* argv[])
{
 ...
 //     printf("getopt_long_only() returned ='%c' index = '%d'\n",  rc, option_index);   
   ...
   if(mode == BISECT)
   {
	DECLARE_TIMER(bisectTimer);
	DECLARE_REPEAT_VAR(bisectR);

	START_TIMER(bisectTimer);
	BEGIN_REPEAT_TIMING(ITER, bisectR);

	iterations = 0;
	result = bisection ( f, guess1, guess2, tol, verbose);

	END_REPEAT_TIMING;
	PRINT_TIMER(bisectTimer);
	PRINT_RTIMER(bisectTimer, ITER);

        printf("Final Bisect Reuslt: %g\n", result);
   ...
   else if(mode == SECANT)
   {
	...
	DECLARE_TIMER(seTimer);
	DECLARE_REPEAT_VAR(seR);

	START_TIMER(seTimer);
	BEGIN_REPEAT_TIMING(ITER, seR);

	result = secant( f, guess1, guess2, maxIter, tol, verbose);

	END_REPEAT_TIMING;
	PRINT_TIMER(seTimer);
	PRINT_RTIMER(seTimer, ITER);

	printf("Final Secant Reuslt: %g\n", result);
   ...
   else /* must be newton */
   {
	...
	DECLARE_REPEAT_VAR(newR);

	START_TIMER(newTimer);
	BEGIN_REPEAT_TIMING(ITER, newR);

	result = newton(f, df, guess1, maxIter, tol, verbose);

	END_REPEAT_TIMING;
	PRINT_TIMER(newTimer);
	PRINT_RTIMER(newTimer, ITER);
	printf("Final Newton Reuslt: %g\n", result);
