...
DECLARE_REPEAT_VAR(repeat1);
   ...
   lcv_X = Data_X.begin();
   lcv_Y = Data_Y.begin();
	START_TIMER(timerCalc);
	BEGIN_REPEAT_TIMING(10000,repeat1);
   ...
   CoefficientsComputed = 1; // reminiscent of C !!
    	...
    	END_REPEAT_TIMING;
	STOP_TIMER(timerCalc);
	PRINT_TIMER(timerCalc);
	PRINT_RTIMER(timerCalc,10000);
	RESET_TIMER(timerCalc);
...
DECLARE_REPEAT_VAR(repeat2);
   ...
   if (1 != argc)
	...
	PRINT_TIMER(timerData);
	PRINT_RTIMER(timerData,5);
	...
	//START_TIMER(timerCalc);
	//BEGIN_REPEAT_TIMING(20000,repeat1);
