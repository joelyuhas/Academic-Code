...
;---------------------------------------------------------------
;---------------------- HEADER -------------------------
;Program outputs initial string, waits for user input
;then, depending on specified input, will call specific
;subroutines
;
...
;----------GetChar
;
;the program also used registers R0-R5, R7, PC, LR
;the program is continually running until manually ended
...
Back										;back from uppercase fix
			CMP		R0,	#'g'				;checking if 'g'
			BEQ		CorrectG				
			CMP		R0, #'i'				;checking if 'i'
			BEQ		CorrectI
			CMP		R0, #'l'				;checking if 'l'
			BEQ		CorrectL
...
CorrectG
			...
			MOVS	R0,#'<'					;printing '>'
			BL		PutChar
			LDR		R0,=String				;loads string
			MOVS	R1,#MAX_STRING			;loads buffer capacity
			BL		GetStringSB				;calls GetStringSB
...
CorrectI
			BL		Correct					;Base correct subroutine that sets up subroutines
			MOVS	R1,#0					;making a zero
			LDR		R0,=String				;loading string
			STRB	R1,[R0,#0]				;killing everyone String ever loved and replacing it with 0
...
CorrectL
			...
			LDR		R0,=String				;loading string
			MOVS	R1,#MAX_STRING			;loading buffer capacity
			BL		LengthStringSB			;calling LengthStringSB
			
			LDR		R0,=Length				;loading length string "length: "
			BL		PutStringSB				;printing it
			
			MOVS	R0,R2					;setting put PutNumU
			BL		PutNumU					;calling PutNumU
...
CorrectP
			...
			LDR		R0,=String				;loading string
			MOVS	R1,#MAX_STRING			;loading buffer cpacity	
			BL		PutStringSB				;printing string
...
InitString	DCB	 "Type a string command (G,i,l,p): ",0
