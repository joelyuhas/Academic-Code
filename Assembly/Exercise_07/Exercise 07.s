...
CorrectP
			...
			MOVS	R7,#0
			CMP		R2,R5
			BEQ		EQUAL_P					;branches if there is an exceptinon wher in_ptr and
											;out_ptr equal but there are items in the queue, so
											;need to traverse it
Loop_P		
			CMP		R5,R2					;checks if out_ptr = in_ptr
			BEQ		DONE_P
			CMP		R4,R7					;checks if counter equals num_enqud
			BEQ		DONE_P

Excepetion									;skips previouse in case there was and exception
			CMP		R5,R3
			BEQ		OVER_P
...
;---------------------- InitQueue ------------------
;initilizes the queue, R1 must be queue record
InitQueue
...
;--------------------------- PutNumHEx---------------
;converts address into hex value
PutNumHex				
		PUSH 	{R0-R5,LR}
...
InitString	DCB	 "Type a queue command (d,e,h,p,s): ",0
