...
AddIntMultiU
		...
		PUSH 	{R3-R7,LR}
		MOVS	R6,#0			;counter
		MOVS	R7,#0			;initialize carry
		SUBS	R3,R3,#1
		LSLS	R3,#2			;incrmennts counter
		
Loop_AIMU
		LDR		R4,[R2,R3]
		LDR		R5,[R1,R3]
		
		MSR		APSR,R7			;putting the carry back
	
		ADCS	R5,R5,R4
		
		MRS		R7,APSR			;perserving the carry
		
		STR		R5,[R0,R3]
		
		
		CMP		R6,R3			;checks to see if the size is the same
		BEQ		DONE
