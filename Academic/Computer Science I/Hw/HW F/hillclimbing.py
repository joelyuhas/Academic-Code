...
class DPoint(rit_object):
    """
    creates class using rit_object
    """
    __slots__ =  ( 'x', 'y', 'z' )
...
def evaluate( x, y ):
    ...
    uses the input values to create a float of the z value
    """
...
def maxPoint( pointList ):
    """
    maxPoint: list-> float
    takes in list, finds largest value of that list
    """
    o = pointList[0]
    for i in pointList:
        if i.z > o.z:
...
def hillClimb( x, y, d ):
    """
    hillClimb: float, float, float, -> list
    takes values of neighboring areas based on d and puts values in list
    """
    zz = float(0)
    lst = []
...
def main():
    """
    complies the values and takes in inputs while also printing them
    """
