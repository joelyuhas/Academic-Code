"""
$Id: rit_object.py,v 2.5 2014/11/16 03:13:55 jeh Exp $
version 2.5b with docs for both version 2 usage and version 1 _types usage
...
NOTE!
   In this version the old style is also supported. In that model, the
   names of a class's slots are defined as a sequence of strings in the
   __slots__ class variable and their corresponding expected types are
   defined in a sequence in the _types class variable.
...
##########################################################################
#                                                                        #
# Definition of rit_object class -- does all the setup and checking      #
#                                                                        #
##########################################################################
...
class rit_object( object ):
    """ The base class for all classes created using this framework.
        ...
        rit_object expects its subclasses to have one of the following
        ...
        Example:
            class Student( rit_object ):
                __slots__ = ( "age", "gpa" )
        If _types is defined in addition to __slots__, then
        each item in _types specifies the type of its 
        corresponding entry in __slots__.
        Example:
            class Student( rit_object ):
                __slots__ = ( "age", "gpa" )
                _types    = ( int, float )
        ...
        value is the name of the slot, as in the __slots__ class.
    ...
    def __init__( self, *args, **kwargs ):
        """ Initialize a new instance of a subclass of rit_object. The
        ...
        if ( thisClass == rit_object ):
            raise TypeError( "rit_object itself may not be instantiated." )
        ...
        if not thisClass._typesScanned: # Do this upon FIRST instance creation.
            ...
            if "__slots__" in dir( thisClass ):
                if "_slots" in dir( thisClass ):
                    raise TypeError( "rit_object subclasses may not have " + \
                ...
                if "_types" in dir( thisClass ):
                    # stderr.write( "rit_object warning: '_types' " + \
            ...
            if "_slots" not in dir( thisClass ):
                raise TypeError( "rit_object subclasses must have " + \
    ...
    def __eq__( self, other ):
        ...
        return rit_object._equal( self, other, visited )
    ...
    def _equal( o0, o1, visited ):
        ...
        if not isinstance( o0, rit_object ): return o0 == o1
        visited.add( ( id0, id1 ) )
        for slotName in o1.__class__._slots: # keys of dictionary
            ...
            if ( sid0, sid1 ) not in visited:
                ...
                if not rit_object._equal( s0, s1, visited ):
                    return False
        return True
    ...
    def __str__( self ):
        """ (DO NOT call this function directly; access it via the str
            ...
            rit_objects, the returned string will be degraded to avoid
    ...
    def __setattr__( self, name, value ):
        ...
        for paramType in paramTypes:
            ...
            if isinstance( value, paramType ):
                ok = True
                break
            # bks added next if clause 3/15/2015
            if isinstance( value, NoneType ) :
                # print( "Checking if", value, "is acceptable", paramType )
                ok = True
                break
        if ok:
...
##########################################################################
#                                                                        #
# quickClass function -- creates an rit_object subclass                  #
#                                                                        #
##########################################################################
...
def quickClass( name, *slotDecls ):
    """ Return a new class that has the provided name and slots (attributes).
        ...
        for rit_object
    """
    return type( name, ( rit_object, ), { '_slots': slotDecls } )
